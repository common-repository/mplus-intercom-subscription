...
if ( ! class_exists( 'Mplus_Intercom_Subscription_Core' ) ) {
	class Mplus_Intercom_Subscription_Core {
		...
		public function __construct() {
			...
			register_activation_hook( MPLUSIS, [ $this, 'activate' ] );
			register_deactivation_hook( MPLUSIS, [ $this, 'deactivate' ] );
		...
		private function mplus_load_dependencies() {
			...
			/**
			 * The class is responsible for showing promo notice
			 *
			 */
			require_once MPLUSIS_PLUGINS_DIR . 'includes/class-mplus-intercom-subscription-promo-notice.php';
		...
		private function mplus_admin_hooks_define() {
			...
			$this->loader->add_action( 'updated_option', $mplusis_settings, 'maybe_delate_api_key', 10, 3 );
			...
			$this->loader->add_action( 'wp_ajax_submit_mplusis_support_form', $plugin_admin, 'submit_mplusis_support_form' );
			$this->loader->add_action( 'admin_init', $plugin_admin, 'promo_notice' );
			...
			$this->loader->add_action("wp_ajax_mplusis_save_settings", $plugin_admin, 'save_settings');
			$this->loader->add_action("wp_ajax_mplusis_create_page_with_shortcode", $plugin_admin, 'create_lead_generation_page');

			$this->loader->add_action("admin_init", $this, 'activation_redirect');

			$this->loader->add_action("wp_die_handler", $plugin_admin, 'redirect_old_license_activation_link_to_new_settings');
		}
		...
		/**
		 * the function will be called after plugin activation
		 */
		public function activate(){
			add_option('mplusis_plugin_activated', 1);
			add_option('mplusis_plugin_activated_at', time());
			mplusis_log("base plugin activated.");
		}

		/**
		 * this function will be called after plugin deactivation
		 */
		public function deactivate(){
			add_option('mplusis_plugin_deactivated', 1);
			add_option('mplusis_plugin_deactivated_at', time());
			mplusis_log("base plugin deactivated.");
		}

		public function activation_redirect(){
			if(get_option('mplusis_plugin_activated')){
				delete_option('mplusis_plugin_activated');
				wp_safe_redirect(admin_url('admin.php?page=mplusis-settings#mplusis_welcome_page'));
				exit;
			}
		}
	}
