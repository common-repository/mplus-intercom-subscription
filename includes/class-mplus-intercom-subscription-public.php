...
if ( ! class_exists( 'Mplus_Intercom_Subscription_Public' ) ) {
	class Mplus_Intercom_Subscription_Public {
		...
		public function mplus_enqueue_styles() {
			wp_enqueue_style( $this->plugin_name, MPLUSIS_PLUGINS_DIR_URI . 'assets/css/mplus-intercom-subscription-public.css', [], $this->version, 'all' );
		}
		...
		public function mplus_enqueue_scripts() {
			wp_enqueue_script( $this->plugin_name, MPLUSIS_PLUGINS_DIR_URI . 'assets/js/mplus-intercom-subscription-public.js', [ 'jquery' ], $this->version, false );

			wp_localize_script( $this->plugin_name, 'wp', [
		...
		public function user_assign_to_company_handler( $new_user, $submitted_fields, $client ) {
			if ( is_object( $new_user ) && isset( $new_user->email ) ) {
				foreach ( $submitted_fields as $field ) {
					if ( array_key_exists( 'name', $field ) && 'company_id' == $field['name'] ) {
